module.exports = {
    ...
    doGA: function (allData, budgetmax_in, budgetmin_in, eliminatedEvents_in) {
        ...
        var lowBudgetFlag = false; // if set to true, ga adds more "none" item+s into the itinerary to allow for a locally optimal itinerary to be found
        var avgFoodCost = parseFloat(parsedDataAll.avgFoodCost);
        var avgEventCost = parseFloat(parsedDataAll.avgEventCost);
        var sumFoodCostEventCost = avgFoodCost + avgEventCost;
        console.log("avgFoodCost= " + avgFoodCost + ", avgEventCost= " + avgEventCost)
        if (budgetmax_in <= LOW_BUDGET_THRESHOLD ||
            sumFoodCostEventCost > 0.65 * budgetmax_in) {
            lowBudgetFlag = true;
            console.log("low budget!!!!")
        }

        if (lowBudgetFlag) // tuning ga for low budget
        {
        ...
        for (var i = 0; i < maxIter; i++) {
            ...
            while (popCnt < popSize) {
                ...
                irand = randomIntFromInterval(1, 100);
                if (irand < mutateRate) {
                    tempItinerary1 = mutate(tempItinerary1, numItemsArray, budgetmax_in, lowBudgetFlag);
                }

                irand = randomIntFromInterval(1, 100);
                if (irand < mutateRate) {
                    tempItinerary2 = mutate(tempItinerary2, numItemsArray, budgetmax_in, lowBudgetFlag);
                }
    ...
    preProcessData: function (allData_in, budgetmax_in) {
        ...
        try {
            ...
            for (var i = 0; i < Math.ceil(numEvent1 / 2); i++) { // half ot the events to keep processing speed up
                avgEventCost = avgEventCost + event1Costs[i];
            }
            ...
            for (i = 0; i < numBreakfast; i++) {
                ...
                avgFoodCost = avgFoodCost + breakfastCosts[i];
            }
            ...
            for (i = 0; i < Math.ceil(numEvent2 / 2); i++) {
                avgEventCost = avgEventCost + event2Costs[i];
            }
            ...
            for (i = 0; i < numLunch; i++) {
                ...
                avgFoodCost = avgFoodCost + lunchCosts[i];
            }
            ...
            for (i = 0; i < Math.ceil(numEvent3 / 2); i++) {
                avgEventCost = avgEventCost + event3Costs[i];
            }
            ...
            for (i = 0; i < numDinner; i++) {
                ...
                if (irand < 3) {
                    ...
                    if (dinnerRatings[i] >= 4.5) {
                        drate = -1;
                    }
                }
                else if (irand === 6) {
                    if (dinnerRatings[i] <= 4) {
                        drate = 1;
                    }
                }
                else {
                ...
                avgFoodCost = avgFoodCost + dinnerCosts[i];
            }
            ...
            for (i = 0; i < Math.ceil(numEvent4 / 2); i++) {
                avgEventCost = avgEventCost + event4Costs[i];
            }
            ...
            // Calculate some random stats
            if (numBreakfast + numLunch + numDinner > 0) {
                avgFoodCost = avgFoodCost / (numBreakfast + numLunch + numDinner);
            }
            else {
                avgFoodCost = 0.0;
            }
            if (numEvent1 + numEvent2 + numEvent3 + numEvent4) {
                avgEventCost = avgEventCost / (Math.ceil(numEvent1 / 2) + Math.ceil(numEvent2 / 2) + Math.ceil(numEvent3 / 2) + Math.ceil(numEvent4 / 2));
            }
            else {
                avgEventCost = 0.0;
            }
...
function mutate(itinerary_in, numItemsArray_in, budgetmax_in, lowBudgetFlag_in) {
    ...
    if (irand <= 6) {
        ...
        if (lowBudgetFlag_in) {
            ...
            var iRandForNoneItem = randomIntFromInterval(0, numItemsArray_in[irand]);
            if (iRandForNoneItem <= Math.ceil(.25*numItemsArray_in[irand]) ) { // 25% chance to be a none item 
                mutatedItinerary[irand] = iNoneItinItem;
            }
            else { // else mutate to something else
